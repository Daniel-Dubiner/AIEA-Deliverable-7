...
def main():
    ...
    for episode in range(EPISODES):
        ...
        with torch.no_grad():
            _, _, next_value = model.get_action(torch.FloatTensor(state).unsqueeze(0).to(device))
    ...
    plt.title("PPO from Scratch on CarRacing-v3")
    plt.legend()
    plt.savefig("/home/ubuntu/persistent/ppo_results.png")
    plt.close()
    print("Plot saved to ppo_results.png")
